// utils/firebase.client.server.ts
...
/**
 * Initialize Firebase client app and DB
 */
function getFirebaseApp(): { app: ReturnType<typeof initializeApp>; db: ReturnType<typeof getDatabase> } {
    ...
    if (!firebaseConfig.apiKey) throw new Error("Firebase client environment config not set");

    app = getApps().length ? getApp() : initializeApp(firebaseConfig, 'client');
    db = getDatabase(app);
