...
import { useLoaderData, useActionData, Form, Link, useSearchParams, useNavigation, useFetcher } from '@remix-run/react';
...
import { Plus, Trash2, Edit, MoreVertical, Eye, EyeOff, Search, FileText, Archive } from 'lucide-react';
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const where: any = { authorId: mongoAuthorId };
  ...
  return json({ posts, total, page, totalPages, mongoAuthorId });
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  // Verify ownership using MongoDB ObjectId
  const post = await prisma.post.findFirst({
    where: { id, authorId: authorRecord.id },
  });
  ...
  try {
    ...
    if (intent === 'archive') {
      ...
      return json({ success: true, message: 'Post archived' });
    }
...
export default function AuthorPosts() {
  ...
  // Helper function for programmatic form submission
  const handleAction = (postId: string, intent: string) => {
    if (intent === 'delete' && !confirm('Delete this post?')) return;
    fetcher.submit({ id: postId, intent }, { method: 'post' });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">My Posts</h1>
          <p className="text-zinc-500 dark:text-zinc-400">{total} total posts</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white dark:bg-zinc-900 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto">
          ...
          <Button
            ...
            className={searchParams.get('status') === 'archived' ? 'bg-zinc-200 text-zinc-700 dark:bg-zinc-700 dark:text-zinc-300' : ''}
          >
            Archived
          </Button>
        </div>
                  ...
                  <TableCell className="font-medium max-w-md">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-zinc-500 mt-1 line-clamp-1">/{post.slug}</div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <Link to={`/authors/post/edit/${post.slug}`}>
                          <DropdownMenuItem>
                        ...
                        {post.status === 'published' ? (
                          <DropdownMenuItem 
                            ...
                            onSelect={() => handleAction(post.id, 'unpublish')}
                          >
                          ...
                          <DropdownMenuItem 
                            className="cursor-pointer"
                            onSelect={() => handleAction(post.id, 'publish')}
                          >
                        ...
                        {post.status !== 'archived' && (
                          <DropdownMenuItem 
                            className="cursor-pointer"
                            onSelect={() => handleAction(post.id, 'archive')}
                          >
                        ...
                        <DropdownMenuItem 
                          ...
                          onSelect={() => handleAction(post.id, 'delete')}
                        >
              ...
              <TableRow>
                <TableCell colSpan={5} className="text-center py-12">
                  <div className="flex flex-col items-center justify-center text-zinc-500">
                    ...
                    <p className="text-sm">
                      {searchTerm ? `No results for "${searchTerm}"` : 'Get started by creating your first post.'}
                    </p>
                  </div>
                </TableCell>
              </TableRow>
