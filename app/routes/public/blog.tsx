...
export const loader: LoaderFunction = async () => {
  try {
    // Import announcements service dynamically to avoid issues if prisma not ready
    let announcements: AnnouncementData[] = [];
    try {
      const { getActiveAnnouncements } = await import("~/Services/announcement.prisma.server");
      announcements = await getActiveAnnouncements();
    } catch {
      // If announcement model doesn't exist yet, use empty array
      announcements = [];
    }

    const [{ posts }, allCategories, portfolioData] = await Promise.all([
    ...
    const postsWithContent = (
      await Promise.all(
        posts.map(async (post) => {
          ...
          // Map categoryIds to partial Category objects
          ...
          return {
            _id: post.id || post._id?.toString(), // Handle Prisma id vs Mongoose _id
            title: post.title,
            slug: post.slug,
            content: content || "",
    ...
    const socials: SocialLinkItem[] = Object.entries(socialLinksObj)
      .map(([id, href]) => ({ id, href: String(href) }))
      .filter((social) => social.href);

    return json<LoaderData>(
      { posts: postsWithContent, categories, socials, announcements },
      {
        headers: {
...
// Utility Components
const SocialIcon = ({ social }: { social: SocialLinkItem }) => {
...
const TopCards = ({ announcements }: { announcements: AnnouncementData[] }) => {
  const eventsCard = announcements.find(a => a.slot === "events");
  const newsCard = announcements.find(a => a.slot === "news");

  const renderCard = (card: AnnouncementData | undefined, fallbackText: string) => (
    <MotionSection className="relative rounded border text-center items-center flex flex-col justify-center h-32 md:h-44 border-indigo-200 dark:border-indigo-500/40 shadow-sm overflow-hidden">
      <img
        src={card?.backgroundUrl || Vanta}
        alt={card?.title || fallbackText}
        className="absolute inset-0 w-full h-full object-cover "
      />
      <div className="absolute inset-0 dark:bg-black/40 bg-white/90 " />
      <div className="relative z-10 px-4">
        <h3 className="text-lg md:text-xl dark:text-zinc-600 text-zinc-400 font-semibold">
          {card?.title || fallbackText}
        </h3>
        {card?.description && (
          <p className="text-sm text-zinc-200 mt-1">{card.description}</p>
        )}
        {card?.linkUrl && (
          <a 
            href={card.linkUrl} 
  ...
  return (
    <div className="max-w-3xl mx-auto mb-8 md:mb-12 px-4 md:px-0">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
        {renderCard(eventsCard, "No Events")}
        {renderCard(newsCard, "No News")}
      </div>
    </div>
  );
};
...
const HeroSection = ({ socials }: { socials: SocialLinkItem[] }) => (
...
export default function Homepage() {
  const { posts, socials, announcements } = useLoaderData<LoaderData>();
  ...
  return (
    <div className="min-h-screen">
      ...
      <TopCards announcements={announcements} />
