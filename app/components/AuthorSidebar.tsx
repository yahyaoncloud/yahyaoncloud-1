...
const authorNavItems: NavItem[] = [
  { name: "Dashboard", href: "/authors/dashboard", icon: Home },
  {
    name: "My Posts",
    icon: FileText,
    children: [
      { name: "All Posts", icon: FileText, href: "/authors/posts" },
      { name: "Create Post", icon: PlusCircle, href: "/authors/post/create" },
    ],
  },
  { name: "My Assets", href: "/authors/assets", icon: Image },
  { name: "My Profile", href: "/authors/profile", icon: User },
];
...
function SidebarNavContent({ onClose, authorName }: { onClose?: () => void; authorName?: string }) {
  return (
    <div className="flex flex-col h-full p-4 overflow-y-auto">
      ...
      <div className="flex items-center mb-6 justify-start gap-4 px-2">
        ...
        <div>
          ...
          <span className="text-xs text-zinc-500 lacquer-regular">Author Portal</span>
        </div>
      </div>

      {/* Author Info */}
      {authorName && (
        <div className="mb-4 px-3 py-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg">
          <p className="text-xs text-indigo-600 dark:text-indigo-400">Logged in as</p>
...
export default function AuthorSidebar({ isOpen, onClose, authorName }: AuthorSidebarProps) {
  const location = useLocation();
  const isLogoutPage = location.pathname === "/authors/logout";
  ...
  return (
    <>
      ...
      >
        <SidebarNavContent onClose={onClose} authorName={authorName} />
      </motion.aside>
