...
export default function Header() {
  ...
  return (
    <header className="relative py-2 mb-8 md:mb-10 text-sm text-zinc-600 dark:text-zinc-400">
      <div className="flex items-center justify-between">
        {/* Title & Breadcrumbs */}
        <div className="flex items-center gap-2 min-w-0">
          <Link
            to="/"
            className="font-semibold text-zinc-900 dark:text-zinc-100 text-base px-1.5 py-0.5 -ml-1.5 rounded hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-all duration-150 active:scale-95"
          >
            Yahya
          </Link>
          {subpage && (
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                to={link.href}
                className={`px-3 py-1 rounded transition-all duration-150 active:scale-95 ${
                  active
